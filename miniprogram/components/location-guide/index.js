...
Component({
  ...
  methods: {
    ...
    calculateScrollHeight: function () {
      ...
      query.select('.location-guide-content').boundingClientRect();
      query.exec((res) => {
        if (res[0] && res[1]) {
          // 成功查询到头部和容器
          ...
          parentQuery.selectAll('.location-guide-modal').boundingClientRect();
          parentQuery.exec((parentRes) => {
            let modalHeight = screenHeight * 0.9; // 默认值：90vh
            ...
            console.log('位置指引滚动区域高度计算（使用模态框实际高度）:', {
              screenHeight,
              modalHeight,
              headerHeight,
              availableHeight,
              scrollHeight,
              '查询结果': parentRes[0] && parentRes[0].length > 0 ? '查询到模态框' : '使用默认值90vh'
            });
